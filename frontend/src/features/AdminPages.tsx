...
export function StaffManagementPage() {
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <TableFrame><table className="w-full min-w-[850px] text-sm"><thead className="border-b border-[#D2D2D7] bg-[#F5F5F7]"><tr>{['Staff Member', 'Email', 'Phone', 'Assigned Branches', 'Status', 'Last Login', 'Actions'].map(label => <TableHead key={label}>{label}</TableHead>)}</tr></thead><tbody>{filtered.length === 0 ? <EmptyRow columns={7}>No matching staff accounts.</EmptyRow> : filtered.map(item => <tr key={item.id} className="border-b border-[#D2D2D7] last:border-0 hover:bg-[#F5F5F7]"><td className="px-4 py-3"><div className="flex items-center gap-2.5"><Avatar name={item.name} tone="violet" /><span className="whitespace-nowrap font-medium text-[#1D1D1F]">{item.name}</span></div></td><td className="px-4 py-3 text-[#6E6E73]">{item.email}</td><td className="whitespace-nowrap px-4 py-3 text-[#6E6E73]">{item.phone || '-'}</td><td className="px-4 py-3"><div className="flex flex-wrap gap-1">{item.branchNames.map(branch => <span key={branch} className="rounded-full border border-[#D2D2D7] bg-[#F5F5F7] px-2.5 py-1 text-xs font-medium">{branch}</span>)}</div></td><td className="px-4 py-3"><StatusBadge status={item.status} /></td><td className="whitespace-nowrap px-4 py-3 text-[#6E6E73]">{shortDate(item.lastLoginAt)}</td><td className="px-4 py-3"><Btn variant="secondary" onClick={() => setEditing(item)} className="px-3 py-1.5 text-xs">Edit</Btn></td></tr>)}</tbody></table></TableFrame>
...
function StaffEditor({ item, branches, onClose, onSaved, onError }: { item: StaffMember | null; branches: BranchManagement[]; onClose: () => void; onSaved: () => void; onError: (error: string) => void }) {
  const [form, setForm] = useState({ firstName: item?.name.split(' ')[0] ?? '', lastName: item?.name.split(' ').slice(1).join(' ') ?? '', displayName: item?.name ?? '', email: item?.email ?? '', phone: item?.phone ?? '', password: '', status: item?.status ?? 'active', branchIds: item?.branchIds ?? [], primaryBranchId: item?.branchIds[0] ?? '' })
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      const body = item ? { firstName: form.firstName, lastName: form.lastName, displayName: form.displayName || null, phone: form.phone || null, status: form.status, branchIds: form.branchIds, primaryBranchId: form.primaryBranchId } : { ...form }
